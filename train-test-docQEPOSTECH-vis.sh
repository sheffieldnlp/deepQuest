...
PARSED_OPTIONS=$(getopt -n "$0"  -o h --long "help,pred-task:,pred-target:,pred-source:,est-task:,est-target:,est-source:,visual:,docsize:,score:,activation:,vis_method:,attention:,seed:,device:"  -- "$@")
...
# we copy the base config
est_conf=config-docQEPOSTECH-vis.py
if [ "${attention}" = true ]; then
	est_model_type=EstimatorDocAtt
else
	est_model_type=EstimatorDocVis
fi
...
CUDA_VISIBLE_DEVICES=0 PYTHONHASHSEED=0 python main.py TASK_NAME=$est_task_name DATASET_NAME=$est_task_name DATA_ROOT_PATH=examples/${est_task_name} SRC_LAN=${est_src} TRG_LAN=${est_trg} VISUAl_FEATURE=${vis} PRED_SCORE=$score OUT_ACTIVATION=$out_activation PRED_VOCAB=$pred_vocab PRED_WEIGHTS=$pred_weights MODEL_TYPE=$est_model_type MODEL_NAME=$est_model_name STORE_PATH=$est_store_path VISUAL_FEATURE_STRATEGY=$vis_strat VISUAL_FEATURE_METHOD=$vis_method PATIENCE=$patience SAVE_EACH_EVALUATION=True RND_SEED=$rnd_seed > log-${est_model_name}-prep.txt 2>&1
