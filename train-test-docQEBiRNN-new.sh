...
CUDA_VISIBLE_DEVICES=0 PYTHONHASHSEED=0 python main.py TASK_NAME=$task_name DATASET_NAME=$task_name DATA_ROOT_PATH=examples/${task_name} SRC_LAN=${src} TRG_LAN=${trg} PRED_SCORE=$score OUT_ACTIVATION=$out_activation MODEL_TYPE=$model_type MODEL_NAME=$model_name STORE_PATH=$store_path SECOND_DIM_SIZE=$docsize PATIENCE=$patience SAVE_EACH_EVALUATION=True RND_SEED=$rnd_seed BATCH_SIZE=$batch_size > log-${model_name}-prep.txt 2>&1
#THEANO_FLAGS=device=$device PYTHONHASHSEED=0
...
rm -rf config.*
ln -s ../config-docBiRNN-new.py ./config.py

# Evaluation on the new test set
CUDA_VISIBLE_DEVICES=0 PYTHONHASHSEED=0 python main.py TASK_NAME=$task_name DATASET_NAME=$task_name DATA_ROOT_PATH=examples/${task_name} SRC_LAN=${src} TRG_LAN=${trg} PRED_SCORE=$score OUT_ACTIVATION=$out_activation MODEL_TYPE=$model_type MODEL_NAME=$model_name STORE_PATH=$store_path SECOND_DIM_SIZE=$docsize PRED_VOCAB=$pred_vocab RELOAD=$best_epoch MODE=sampling PATIENCE=$patience SAVE_EACH_EVALUATION=True RND_SEED=$rnd_seed BATCH_SIZE=$batch_size > log-${model_name}.txt 2>&1
#THEANO_FLAGS=device=$device PYTHONHASHSEED=0
